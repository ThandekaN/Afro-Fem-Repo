...
let selectedLevelButtons = levels.find((level) => {
    const classList = Array.from(level.classList);
    return classList.includes("selected");
});

let gameLevel = selectedLevelButtons.innerHTML;
...
//Attempt to make all the squares have a backgroung color: rgb(200, 45, 255)
...
//assign each individual square a background color
    ...
    //assign the Header a random rgb value from on the square values.
...
//add event listener to each squares so that it either dissappears or changes every square's color
